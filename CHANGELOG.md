 ...
 * Refactor the whole Lygneo sourcecode + clean old code
 * Quite all the Lygneo widgets are now using a full MVC system
 * Rewrite the core session manager (Sessionx) 
 * Add a new localisation system + new translations
 * Move the Lygneo librairies and dependencies to Composer and convert Modl and Moxl to PSR-0 to simplify the loading and packaging of the libraries
 * Monolog is now the new log library for Lygneo
 ...
 * Implement the new official Lygneo API (https://api.lygneo.eu/)
 ...
 * List all the Lygneo network pods on a new page
 ...
 * Parse all the Lygneo messages to make them more “user-friendly” (smileys, links, bb-code like) @Etenil
 * DONE Make a public XML page reporting on the pod status (how many user hosted, version, current status…), to be pinged from pod.lygneo.eu @edhelas
 ...
 * Test Lygneo on all Datajar back-ends @etenil
 ...
 * Inscription on the Server (XMPP+Lygneo)
