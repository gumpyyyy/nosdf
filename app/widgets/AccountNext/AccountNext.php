...
class AccountNext extends WidgetBase {
    ...
    function ajaxDiscoverServer($ndd) {
        try {
            ...
            if(!$f ) {
                RPC::call('lygneo_reload', Route::urlize('account', 'xmppconnect'));
                RPC::commit();
                exit;
            }
            ...
            if(!$response) {
                    RPC::call('lygneo_reload', Route::urlize('account', 'xmppcomm'));
                    RPC::commit();
                     exit;
            }
            ...
            if(!empty($elements)) {
                ...
                $formview->assign(
                    'submitdata',
                    $this->genCallAjax('ajaxSubmitData', "lygneo_form_to_json('data')"));
            ...
            RPC::call('lygneo_fill', 'subscription_form', $html);
            RPC::commit();
    ...
    function ajaxSubmitData($datas) {
        ...
        try {         
            ...
            if(!$f) {
                RPC::call('lygneo_reload', Route::urlize('accountnext', 'xmppconnect'));
                RPC::commit();
                 exit;
            }
            ...
            if(!$response) {
                    RPC::call('lygneo_reload', Route::urlize('accountnext', array(XMPP_HOST, 'xmppcomm')));
                    RPC::commit();
                     exit;
            }
            ...
            if($iq->error) {
                ...
                if($cond->getName() == 'conflict') {
                    RPC::call('lygneo_reload', Route::urlize('accountnext', array(XMPP_HOST,'userconflict')));
                    RPC::commit();
                     exit;
                } else if($cond->getName() == 'not-acceptable') {
                    RPC::call('lygneo_reload', Route::urlize('accountnext', array(XMPP_HOST,'notacceptable')));
                    RPC::commit();
                     exit;
                }
            ...
            if($iq = $response->iq and $iq->attributes()->type == 'result') {
                RPC::call('lygneo_reload', Route::urlize('login', 'acccreated'));
                RPC::commit();
                exit;
            } else {
                    RPC::call('lygneo_reload', Route::urlize('accountnext', array(XMPP_HOST,'unknown')));
                    RPC::commit();
                     exit;
            }
        } catch(Exception $e) {
