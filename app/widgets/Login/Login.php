...
class Login extends WidgetBase
{
    ...
    function display()
    {
        $submit = $this->genCallAjax('ajaxLogin', "lygneo_parse_form('login')");
    ...
    function onMoxlError($error) {
        RPC::call('lygneo_redirect', Route::urlize('disconnect', $error[1]));
    }
    ...
    private function displayWarning($warning, $htmlonly = false)
    {
        if($warning != false) {
            ...
            if($htmlonly)
                ...
                RPC::call('lygneo_fill', 'warning', $warning);
    ...
    function ajaxLogin($element)
    {
        ...
        if($warning != 'OK') {
            RPC::call('lygneo_redirect', Route::urlize('login', $warning));
            RPC::commit();
        } else {
            $pd = new modl\PresenceDAO();
            $pd->clearPresence($element['login']);
        
            RPC::call('lygneo_remember_session', $element['login']);
            RPC::call('lygneo_reload', Route::urlize('root'));
            RPC::commit();
        }
    }
    ...
    function ajaxGetRememberedSession($sessions)
    {
        ...
        RPC::call('lygneo_fill', 'sessions', $sessionshtml->draw('_login_sessions', true));
        RPC::commit();
    }
