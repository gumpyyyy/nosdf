...
class ServerNodes extends WidgetCommon
{
    ...
    {
        RPC::call('lygneo_fill', 'servernodeshead', '');
    }
    
    function onCreationSuccess($items)
    {        
        RPC::call('lygneo_redirect', Route::urlize('node', array($items[0], $items[1])));        
    }
    
    function onCreationError($error) {
        RPC::call('lygneo_fill', 'servernodes', '');
        RPC::commit();
    }
    ...
    function onDiscoItems($server) {
        $submit = $this->genCallAjax('ajaxCreateGroup', "lygneo_parse_form('groupCreation')");
        ...
        if(!in_array($type, array('conference', 'muc', 'discussion', 'chat'))) {
            $head = '
                <a 
                    ...
                    onclick="lygneo_toggle_display(\'#groupCreation\')">
        ...
        RPC::call('lygneo_fill', 'servernodeshead', $head);
        RPC::call('lygneo_fill', 'servernodeslist', $html);
        RPC::commit();
    }
