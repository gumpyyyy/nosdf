...
class Node extends WidgetCommon
{
    ...
    {        
        ...
        RPC::call('lygneo_fill', 'node', $html);
    ...
    function onPubsubAffiliations($params) {
        ...
        && ($this->role == 'owner' || $this->role == 'publisher')) {
            RPC::call('lygneo_toggle_display', '#formpublish');
            RPC::call('lygneo_toggle_display', '#configbutton');
        }
    }

    function onPubsubMetadata($params) {
        $html = $this->prepareMetadata($params[0], $params[1]);
        RPC::call('lygneo_fill', 'metadata', $html);
    }
    ...
    function onStream($payload) {
        ...
        RPC::call('lygneo_fill', md5($payload['from'].$payload['node']), $html);
    }
    ...
    function prepareNode($serverid, $groupid) {
        ...
        $nodeview->assign('subscribe',      $this->genCallAjax('ajaxSubscribe', "lygneo_parse_form('groupsubscribe')", "'".$serverid."'", "'".$groupid."'"));
    ...
    function ajaxGetPostsNode($serverid, $groupid, $start) {
        ...
        RPC::call('lygneo_append', md5($serverid.$groupid), $html);
        RPC::commit();
    }
