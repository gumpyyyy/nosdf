...
class NodeConfig extends WidgetBase
{
    ...
    function onGroupDeleted($server) {
        ...
        RPC::call('lygneo_fill', 'handlingmessages', $html);
        RPC::commit();        
    }
    ...
    function onConfigForm($form) {
        $submit = $this->genCallAjax('ajaxSubmitConfig', "lygneo_parse_form('config')", "'".$form[1]."'", "'".$form[2]."'");
        $html = '
            <form name="config">'.
                $form[0].
                '
        ...
        RPC::call('lygneo_fill', 'groupconfiguration', $html);
        RPC::commit();
    }
