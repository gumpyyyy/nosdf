...
if($argsize == 1) {
    echo 
colorize("Welcome to Mud - Lygneo Unified Doer

Here some requests you can do with me :", 'green')."
- ".colorize("getloc", 'yellow')."    grab all the translations in Lygneo and generate a global locale file
- ".colorize("comploc", 'yellow')."   compile the current locales to a simple PHP array to boost Lygneo execution
- ".colorize("comptz", 'yellow')."    compile the timezones
- ".colorize("db", 'yellow')."        create/update the database
- ".colorize("config", 'yellow')."    set the configuration of Lygneo (separated by commas and colons) eg. info:Test,description:Hop
        ";
...
function config($values) {
    echo colorize("Lygneo configuration setter\n", 'green');
